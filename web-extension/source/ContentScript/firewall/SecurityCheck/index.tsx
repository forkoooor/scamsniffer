...
import { BalanceChange, CheckResponse } from "../shared/types";
...
function RenderBalanceTitle({ change }: {
    change: BalanceChange
}) {
    return <a href={change.detail} target="_blank" style={{
        color: "inherit",
        textDecoration: "none"
    }}>{
            change.tokenId ? `${change.name} #${change.tokenId}` : change.name
        }</a>
}


function RenderBalanceChange(props: {
    ...
    return (
        <List style={{
            ...
            <div style={{ fontSize: "0.9rem", margin: "15px 0 10px" }}>Balance Changes</div>
            {balanceChange.filter(c => c.isSigner).map((change: BalanceChange, index: any) => {
                return (
                    <ListItem key={index} secondaryAction={
                        <span style={{
                            fontSize: "0.95rem",
                            color: change.direction === 'in' ? "#0eed0e" : "rgb(236, 46, 46)"
                        }}>
                            {change.direction === 'in' ? '+ ' : '- '}
                            {change.amount}
                        </span>
                        ...
                        <ListItemAvatar>
                            <Avatar src={change.image} sx={{ width: 28, height: 28 }} />
                        </ListItemAvatar>
                        <ListItemText
                            secondary={<RenderBalanceTitle change={change} />}
                        />
                    </ListItem>
                );
            })}
...
function getRandomTip() {
    ...
    const drops = appConfig.remoteConfig && appConfig.remoteConfig.drops || [];
    const tipOnew = drops[Math.floor(Math.random() * drops.length)];
    return tipOnew
}


function isTranscationSimulationDisabled() {
    const appConfig = getConfig();
    return appConfig.transcationSimulationDisabled || false;
}
...
export default function Content(props: Props) {
    ...
    const isTxDisabled = isTranscationSimulationDisabled();
    useEffect(() => {
    ...
    const hasBalanceChange = checkResponse && checkResponse.balanceChange.filter(c => c.isSigner).length && !isTxDisabled
    return (
        ...
        >
            <DialogTitle
                ...
                style={{
                    color: "white",
                    textAlign: "center",
                    padding: "padding: 8px 24p"
                }}
            >
                <HeaderLogo />
            </DialogTitle>
            <DialogContent style={{ padding: "0", height: "100%" }} dividers>
                        ...
                        <div style={{ textAlign: "center", color: "#f4f4f4" }}>
                            <div style={{ padding: "10px 0 18px 0" }}>
                                <div style={{ paddingTop: "7px" }}>
                                    {checking ? (
                                        <CircularProgress />
                                    ) : checkListResult.filter((_: any) => _.status === 1).length ? (
                                        <WarningAmberIcon
                                            style={{ fontSize: "2.5rem", color: "#ec2e2e" }}
                                        />
                                    ) : checkListResult.filter((_: any) => _.status === 2).length ? (
                                        <WarningAmberIcon
                                            style={{ fontSize: "2.5rem", color: "yellow" }}
                                        />
                                    ) : (
                                        <CheckIcon
                                            style={{ fontSize: "2.5rem", color: "#0eed0e" }}
                                        />
                                    )}
                                </div>
                                <div style={{ fontSize: "0.98rem", margin: "7px 0" }}>
                                    Security Check
                                    <a
                                        ...
                                        <ListItem key={index} secondaryAction={
                                            result.descriptionLink ?
                                                <IconButton edge="end" aria-label="delete">
                                                    <a href={result.descriptionLink} target="_blank"
                                                        style={{
                                                            color: "white",
                                                        }}>
                                                        <HelpOutlineIcon
                                            ...
                                            <ListItemIcon>
                                                {result.status ? (
                                                    ...
                                                    <CheckIcon
                                                        style={{ fontSize: "1.1rem", color: "#0eed0e" }}
                                                    />
                                                )}
                                            </ListItemIcon>
                                            <ListItemText
                            ...
                            {hasBalanceChange ? <Divider variant="middle" /> : null}
                            {hasBalanceChange ? <RenderBalanceChange checkResponse={checkResponse} /> : null}
                        </div>
                    ) : null}
                </div>
                {
                    randomTip ? <Divider variant="middle" /> : null
                }
                {randomTip ? <Box style={{
                    fontSize: '0.5rem',
                    marginTop: "0.8rem",
